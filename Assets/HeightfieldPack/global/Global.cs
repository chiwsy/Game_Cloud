...
//				GUI.EndGroup ();
		...
		void Start ()
		{
			...
			score.pixelOffset=new Vector2(-Screen.width/2+50,Screen.height/2-10);
//		audio.loop = true;
		...
		void FixedUpdate ()
		{
			...
			if(CloudtimePeriod>CloudrefreshTimeBound){
				...
				while(currentCloudNum<CloudNum){
					...
					if(!Physics.Raycast(Cam.transform.position,pos-Cam.transform.position)) continue;
					else{
						currentCloudNum++;
		...
		void Update ()
		{
		score.text=(Mathf.RoundToInt( distance+10.0f*bonus)).ToString();
